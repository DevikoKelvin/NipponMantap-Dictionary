...
class KatakanaAFragment : Fragment() , ItemCallback {

    private lateinit var binding: FragmentKatakanaABinding

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?
    ): View? {

        binding = FragmentKatakanaABinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if(activity != null)
        {
            ...
            val data = viewModel.getListKatakanaA()

            val adapter = HurufAdapter(this)
            adapter.setData(data)

            with(binding.rvKatakanaA){
